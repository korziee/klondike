import { Pile, Card, TSuit, ISerializedPile } from "@cards-js/common";
...
export class FoundationPile extends Pile {
  ...
  canUserRemoveCards(cards: Card[]): boolean {
  ...
  canUserAddCards(cards: Card[]): boolean {
    // early exit if not all same suit, not performant but easy to read!
    const allCardsAreCorrectSuit = cards.every(
      (c) => c.getSuit() === this.suit
    );
