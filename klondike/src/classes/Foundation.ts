import { Pile, Card, TSuit, ISerializable } from "@cards-js/common";
...
export class Foundation implements IFoundation {
  ...
  addCard(card: Card): void {
    if (!this.getPileForSuit(card.getSuit()).canUserAddCards([card])) {
      return;
    }
  ...
  removeCard(card: Card): void {
    ...
    if (!suitFoundationPileClass.canUserRemoveCards([card])) {
      return;
    }
