...
test("cannot remove cards that are not 'upturned'", t => {
  ...
  const canRemoveCards = tableauPile.canRemoveCards([cards[2], cards[3]]);
...
test("cannot remove cards that are not alternating", t => {
  ...
  canRemoveCards = tableauPile.canRemoveCards([
...
test("cannot add cards that are not alternating and sequential", t => {
  ...
  const canAddCards = tableauPile.canAddCards([
    new Card("Clubs", "9", false),
    new Card("Hearts", "5", false),
...
test("cannot add cards even if it just one card", t => {
  ...
  t.is(tableauPile.canAddCards([cardToPlace]), false);
});
...
test("only kings can be placed on an empty TableauPile", t => {
  ...
  canAddCards = tableauPile.canAddCards([new Card("Hearts", "7", true)]);
  t.false(canAddCards);
  canAddCards = tableauPile.canAddCards([new Card("Hearts", "Queen", true)]);
  t.false(canAddCards);
  canAddCards = tableauPile.canAddCards([new Card("Hearts", "Ace", true)]);
  t.false(canAddCards);
  canAddCards = tableauPile.canAddCards([new Card("Hearts", "King", true)]);
  t.true(canAddCards);
});
...
test("any suit of kings can be placed on an empty TableauPile", t => {
  ...
  canAddCards = tableauPile.canAddCards([new Card("Hearts", "King", true)]);
  t.true(canAddCards);
  canAddCards = tableauPile.canAddCards([new Card("Spades", "King", true)]);
  t.true(canAddCards);
  canAddCards = tableauPile.canAddCards([new Card("Clubs", "King", true)]);
  t.true(canAddCards);
  canAddCards = tableauPile.canAddCards([new Card("Diamonds", "King", true)]);
  t.true(canAddCards);
});
...
test("can add cards to the pile if they are alternating and sequential", t => {
  ...
  const canAddCards = tableauPile.canAddCards(cardsToAdd);
  t.true(canAddCards);
